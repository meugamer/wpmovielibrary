 ...
 * developers to output multiple term meta fields within a single metabox.
 *
 * @package    Haricot
...
class Haricot_Manager {
	...
	/**
	 * Taxonomy this manager is used on.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    string|array
	...
	/**
	 * ID of the term that's being edited.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    int
	...
	public function register_section( $section, $args = array() ) {
		...
		if ( ! is_object( $section ) ) {
			...
			$type = isset( $args['type'] ) ? haricot()->get_section_type( $args['type'] ) : haricot()->get_section_type( 'default' );
	...
	public function register_control( $control, $args = array() ) {
		...
		if ( ! is_object( $control ) ) {
			...
			$type = isset( $args['type'] ) ? haricot()->get_control_type( $args['type'] ) : haricot()->get_control_type( 'default' );
	...
	public function register_setting( $setting, $args = array() ) {
		...
		if ( ! is_object( $setting ) ) {
			...
			$type = isset( $args['type'] ) ? haricot()->get_setting_type( $args['type'] ) : haricot()->get_setting_type( 'default' );
	...
	public function save( $term_id ) {
		...
		if ( ! isset( $_POST["haricot_{$this->name}"] ) || ! wp_verify_nonce( $_POST["haricot_{$this->name}"], "haricot_{$this->name}_nonce" ) )
			return;
	...
	public function print_template() { ?>
		...
		<script type="text/html" id="tmpl-haricot-manager-<?php echo esc_attr( $this->type ); ?>">
	...
	public function get_template() {
		haricot_get_manager_template( $this->type );
	}
}
