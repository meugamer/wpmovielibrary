...
<# 	if ( 'movie' === data.settings.get( 'type' ) ) { #>
		<div class="grid-setting-block half-col orderby-setting">
			...
			<span class="grid-setting-input">
				<input id="{{ data.grid_id }}-orderby-post_title" name="{{ data.grid_id }}-orderby[]" type="radio" data-setting-type="orderby" value="title" <# if ( 'title' == data.query.get( 'orderby' ) ) { #>checked="checked" <# } #>/><label for="{{ data.grid_id }}-orderby-title" class="value"><?php _e( 'Title', 'wpmovielibrary' ); ?></label>
			</span>
			<span class="grid-setting-input">
				<input id="{{ data.grid_id }}-orderby-post_date" name="{{ data.grid_id }}-orderby[]" type="radio" data-setting-type="orderby" value="date" <# if ( 'date' == data.query.get( 'orderby' ) ) { #>checked="checked" <# } #>/><label for="{{ data.grid_id }}-orderby-date" class="value"><?php _e( 'Date', 'wpmovielibrary' ); ?></label>
			</span>
		</div>
<# 	} #>
